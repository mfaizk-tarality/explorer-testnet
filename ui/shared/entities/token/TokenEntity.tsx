...
import { Avatar, Image, chakra } from "@chakra-ui/react";
...
const TokenEntity = (props: EntityProps) => {
  ...
  const { approvedTokenList } = useAppContext();

  const getAvatar = () => {
    return approvedTokenList?.find(
      (item) => item?.contractAddress == partsProps?.symbol?.token?.address
    )?.imageUrl;
  };
  ...
  return (
    <Container w="100%" {...partsProps.container}>
      {getAvatar() ? (
        <>
          <Avatar src={getAvatar()} size={"xs"} mr={2} />
        </>
      ) : (
