import { Avatar, Box, Flex, Tooltip, useToken } from "@chakra-ui/react";
...
const TokenPageTitle = ({ tokenQuery, addressQuery, hash }: Props) => {
  ...
  const getAvatar = () => {
    return appProps?.approvedTokenList?.find(
      (item) => item?.contractAddress == tokenQuery?.data?.address
    )?.imageUrl;
  };
